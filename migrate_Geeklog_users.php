...
<ul>
<li>Set Host, Username and Password of Geeklog database same as wordpress database. </li>
<li>Delete users of wordPress site excluding admin user.</li>
<li>To remove duplication of users ID in users table, please make unique ID of admin user ID in user table and user meta table which are not exists in Geeklog users IDs.</li>
</ul>
</p>
</div>
<?php
...
if(isset($_POST['submit'])){
	$GeeklogDBname_DBPrefix = sanitize_text_field($_POST['GeeklogDBname_DBPrefix']);
	$dbExplode 	= explode(".",$GeeklogDBname_DBPrefix);
	$dbName 	= $dbExplode[0];
	...
	if($GeeklogDBname_DBPrefix != 'GeeklogDBname.DBPrefix' ){
		// Check db
		...
		$jUser = $lwpdb->get_results( $lwpdb->prepare( "SELECT
				...
				u.passwd password
				FROM ".$GeeklogDBname_DBPrefix."_users u
				ORDER BY u.uid","",""));
		global $wpdb;
		$wpdb->show_errors();
...
$GeeklogDBname_DBPrefix='GeeklogDBname.DBPrefix';
}
?>
<form method="post">
<table>
<tr><th>Insert Geeklog database name with prefix</th></tr>
<tr><th><span style="color:red;"> (ex - GeeklogDBname.DBPrefix) *</span></th><td><input type="text" name="GeeklogDBname_DBPrefix" id="GeeklogDBname_DBPrefix" onfocus="this.value=='GeeklogDBname.DBPrefix'?this.value='':this.value=this.value;" onblur="this.value==''?this.value='GeeklogDBname.DBPrefix':this.value=this.value;" value="<?php if(isset($GeeklogDBname_DBPrefix)) { echo $GeeklogDBname_DBPrefix; } ?>" maxlength="100" size="30px;"></td></tr>
